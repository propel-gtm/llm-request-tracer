...
func TestSimpleTracker(t *testing.T) {
	mockStorage := &MockStorageAdapter{}
	simpleTracker := NewSimpleTracker(mockStorage)

	t.Run("TrackOpenAI", func(t *testing.T) {
		err := simpleTracker.TrackOpenAI("gpt-3.5-turbo", 50, 100, 500*time.Millisecond, nil)
		if err != nil {
			t.Errorf("Expected no error, got %v", err)
		}
	...
	t.Run("TrackWithDimensions", func(t *testing.T) {
		dimensions := map[string]interface{}{
			"user_id": "user-123",
			"feature": "chat",
		}

		err := simpleTracker.TrackWithDimensions(
			ProviderAnthropic,
			"claude-3-haiku",
			75,
			125,
		...
		if lastReq.Dimensions["user_id"] != "user-123" {
			t.Errorf("Expected user_id user-123, got %v", lastReq.Dimensions["user_id"])
		}
	})
}